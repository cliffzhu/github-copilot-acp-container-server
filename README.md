...
| `ACP_WORKDIR_DOC_SOURCE` | `/opt/acp-library` (image default), empty in Docker Compose | Path to a directory of documents. Startup syncs its contents into `ACP_WORKDIR`, replacing destination files only when the source is newer. `/opt/acp-library` is baked into the image at build time from the local `workspace/` folder, so plain `docker run` deployments ship with the document library out of the box. Docker Compose sets this to empty because `./workspace:/workspace` is already a live bind mount of the same docs. Ignored when empty, missing, or containing no files. |
| `DOC_SYNC_CRON_INTERVAL_HOURS` | _unset_ | Optional recurring document sync interval from 1 to 24 hours. When set, startup creates an in-container cron job that reruns the `ACP_WORKDIR_DOC_SOURCE` update-only sync at that interval and writes activity to the container console. The initial startup sync always runs first. |
